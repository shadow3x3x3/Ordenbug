...
describe Edge do
  describe "#attr_array" do
    it "attr_array should be [5*2, 6, 1*2, 7, 3, 8*2, 4*2]" do
      @edge = Edge.new(*[5, 2, 3, 5, 6, 1, 7, 8, 3, 4], dim_times_array: [2, 1, 2, 1, 2, 1, 2])
      result = @edge.attr_array
       expect(result).to eq([5*2, 6, 1*2, 7, 3, 8*2, 4*2])
    end
  end
end

describe Graph do
  context "no dim times" do
    before(:each) do
